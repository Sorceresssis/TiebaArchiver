# TODO TiebaReader 用 reply_num 来判断是否获取 subpost, 如果不更新 reply_num, TiebaReader会导致新增的subpost无法显示。

# 1. 更新本地的帖子数据, 此过程将帖子新增的数据融入本地数据中。对于已被删除的帖子，会继续保留本地数据。实现数据的持续扩充和更新,最大程度地保留数据。
# 2. 兼顾原始数据结构升级到最新的版本的。
...
# 为什么不适用
# 因为当修改了 post_filter 的配置时。这一个方法将会失效。防止错误操作。
